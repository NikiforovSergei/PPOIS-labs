...
struct T //струстура списка смежности 
{
    Type id;
    T<Type>* Next;
};
...
template<typename Type>
struct L //структура списка заголовочных узлов
{
    Type Key = NULL;
    int Count = NULL;
...
template<typename Type>
class Graph         //класс самого графа
{

private:
    typedef L<Type> value_type; //ссылка на список заголовочных узлов
    typedef value_type* value_pointer; //указатель на него
    typedef T<Type>* edge_pointer; //указатель на список смежности 

    value_pointer Head; //начало списка заг узлов
    value_pointer Tail; //указатель на конец списка заг узлов

    void getKey_or_addKey(Type key, value_pointer& key_pointer) {  // находит вершину  по ключу, если такой вершины нет, то создает ее
        key_pointer = Head; Tail->Key = key;
        while (key_pointer->Key != key) key_pointer = key_pointer->Next; //проверка на ни=аличие
        if (key_pointer == Tail)
        {
            Tail = new value_type;
            key_pointer->Count = 0;
