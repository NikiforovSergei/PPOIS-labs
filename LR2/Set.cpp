...
bool Set::compare_subset(Set* a, Set* b) //сравнивает подмножества двух главных множеств
{
    ...
    for (int i = 0; i < b->subsets.size(); i++)
    {
        ...
        {
            if (!Set::compare_subset(a->subsets[j], b->subsets[i]))
                return false;
            else
                return true;
        }
...
Set* Set::sum_private(Set *Union, Set *b) //Передается уже скопированное множество Union (оно же изначальное множество a)
{
    ...
    {
        if (find(Union->elements.begin(), Union->elements.end(), b->elements[i]) == Union->elements.end()) // если элемента из второго множества нету в первом
        {
...
Set* Set::composition(Set* a, Set* b)
{
    ...
    {
        auto same = find(b->elements.begin(), b->elements.end(), a->elements[i]); // находит элементы одинаковые в обоих множествах
