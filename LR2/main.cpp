...
// Pl - Plankton, Wh - Whale, H_f - Herbivore fish, Pr_f - Predator fish, Sh - Shark
class Cell{
        ...
        if(!is_full()){
            ...
            if(contains("Wh").size() > 1 ){
                ...
                for(int i : contains("Wh")){
                    if(organisms.at(i)->get_sex() == 0) male = true;
                    else if(organisms.at(i)->get_sex() == 1) female = true;
                }
...
public:
    ...
    void print_field(){
        for(std::vector<Cell> &line : field){
            for (Cell cell : line){
                ...
                else std::cout << std::setw(5) << cell.organisms[0]; std::cout << " " ;

                if (cell.organisms[1]) std::cout << std::setw(5) <<  cell.organisms[1]->type();
                else std::cout << std::setw(5) << cell.organisms[1]; std::cout << "\t|\t" ;
            }

            std::cout << "\n";
            for (Cell &cell : line){
                if (cell.organisms[2]) std::cout << std::setw(5) <<  cell.organisms[2]->type();
                else std::cout << std::setw(5)<< cell.organisms[2]; std::cout << " " ;

                if (cell.organisms[3]) std::cout << std::setw(5) << cell.organisms[3]->type();
                else std::cout << std::setw(5)<< cell.organisms[3]; std::cout << "\t|\t" ;
            }
            ...
            for(size_t j = 0; j < field.at(i).size(); j++){
                ...
                for(size_t n = 0; n < field.at(i).at(j).organisms.size(); n++){
                    if(field.at(i).at(j).organisms.at(n) && field.at(i).at(j).organisms.at(n)->superclass() != "Pl"){
                        ...
                        /* Checkout whether the fish have moved within the iteration*/
                        bool used_ptr = false;
                        for(Organism *&o: used)
    ...
    void iteration(bool more_info = false){
        ...
        /* Dying from old age or starving */
        for(std::vector<Cell> &line: field)
            for(Cell &cell: line)
                cell.death();
