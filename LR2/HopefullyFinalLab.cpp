...
//void Input_From_File::GetFileName() {
	//cin >> this->filename;
//}

void Input_From_File::Input(string filename, int& input, int& input_2, int& width, int& height, int& w, int& s, int& h, int& p) {
...
int Ocean_master::empty_place(int x, int y, bool plankton)
{
	if ((x < 0) || (y < 0) || (x >= ocean_table->width) || (y >= ocean_table->height))
		return -1;//выход за границы поля
	if (plankton)
	{
		for (int i = 0; i < planktons.size(); i++)
		{
			if ((planktons[i].location[0] == x) && (planktons[i].location[1] == y))
				return -2; //есть планктон
		}
	}
	for (int i = 0; i < 4; i++)
