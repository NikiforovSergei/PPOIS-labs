...
void Saved_Set::save_sets(string line)
{
    ...
    string my_no_named_set = line.substr(beg_str, end_str);
    Set* found_set = solution(my_no_named_set);
    try
    {

        if (found_set == nullptr)
            throw "set must not store any characters other than letters";
...
Set* Saved_Set::operation_with_named_sets(string my_str)
{
    ...
    for (int i = 0; i < my_str.length(); i++)
    {
        ...
        if (isalpha(my_str[i]))
        {
            ...
            a = my_str[i];
            try {
                if (named_sets.find(a) == named_sets.end()) // проверка на существаование множества
                    throw "non-existent variable!";
...
void Saved_Set::distr(string my_str) // какая из операций
{
    bool check_small = false, eq = false;
    if (my_str.find("=") != -1) eq = true;
    if (my_str.find('{') != -1) check_small = true;

    try
    {
        if (check_small && eq) { // если создание множества
            save_sets(my_str);
            return;
        }

        else if (!check_small && !eq) { // если операция над названным множеством
            get_info(operation_with_named_sets(my_str));
            return;
        }

        else if (!check_small && eq) // операция A = B
        {
