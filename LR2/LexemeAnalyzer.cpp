...
Var vars;
Error::Error(int NUMBER_ERROR) //Constractor
{
	this->NUMBER_ERROR = NUMBER_ERROR;
	cout << "Code of the error: " << NUMBER_ERROR << " ";
	switch (NUMBER_ERROR)
	{
	case 100: cout << "Input an expression" << endl; break;
	case 101: cout << "Invalid symbol" << endl; break;
	case 102: cout << "Invalid input. Missed number/var/operator" << endl; break;
	case 103: cout << "Invalid input. Missed parametrs" << endl; break;
	}
}
...
void LexemeAnalyzer::get_token(LexemeBuffer& lexemes)
{
	while (true)
	{
		...
		switch (ch)
		{
		case 0: case '{': case '}': case '\\': case '[': case ']': case ':': case ';': {Error error(101); cin.ignore(cin.rdbuf()->in_avail()); }
		case '\n':return;
...
double Lexeme::getDValue(Var& vars)
{
	if (isfoo(token))
	{
		...
		switch (token)
		{
		case ASIN: case ACOS: case ATAN: case ACTG: case SQRT: case CBRT: n = 4; break;
		default:n = 3;
			break;
		}
		bool param = false;
		...
		else
		{
			...
			{
			case 3: 
				return sin(parametr[0]) * k;
			case 4: 
				return cos(parametr[0]) * k;
			case 5: 
				return tan(parametr[0]) * k;
			case 6: 
				return 1 / tan(parametr[0]) * k;
			case 7: 
				return asin(parametr[0]) * k;
			case 8: 
				return acos(parametr[0]) * k;
			case 9: 
				return atan(parametr[0]) * k;
			case 10: 
				return PI_2 - atan(parametr[0]) * k;
			case 11: 
				return pow(parametr[0], parametr[1]) * k;
			case 12: 
				return sqrt(parametr[0]) * k;
			case 13: 
				return cbrt(parametr[0]) * k;
			}
		}
		return 0;
	}
