...
private:
    ...
    Rib<value_type>* FindRib(value_type firstKey, value_type secondKey) const
    {
        ...
        while (ribFinder != nullptr)            // Поиск ребра
        {
    ...
    {
        rib->id = key;
        rib->nextRib = node->nextRib;         // Добавление ребра
        node->nextRib = rib;
        node->ribCount++;
    }
    ...
    void DettachRibFromNode(Node<value_type>* node, value_type key)
    {
        ...
        {
            previousRib = firstRibFinder.rib;                       // Поиск ребра с запоминанием
            firstRibFinder++;
        }
        if (previousRib == nullptr)                             // Удаление ребра
        {
...
inline void Graph<T, A>::RemoveNode(value_type key)
{
    ...
    NodeIterator nodeFinder = Head;
    while (nodeFinder->key != key)                      // Поиск нода с запоминанием
    {
    ...
    {
        RemoveRib(nodeFinder->key, rib->id);            // Удаление рёбер
